...
> needed at least one real fix the Docker stack never surfaced (capabilities,
> UID mapping, firewall interactions); see each README's Notes and
> [docs/migration.md](docs/migration.md) for what was actually wrong and how
> it was found. omada additionally required pinning an exact version tag
> (`:latest` resolved to a stale build at deploy time -- see its README), and
> netbird's LAN-routing-peer use case required a rootful deviation + a ufw
> forward-policy fix beyond the base rootless deploy (see netbird's README).
...
| [omada](containers/omada) | TP-Link Omada SDN controller | `docker.io/mbentley/omada-controller:6.2.10.17` | 8043 | `omada-data` / `omada-logs` (volume) |
